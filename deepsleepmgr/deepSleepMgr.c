...
IARM_Result_t DeepSleepMgr_Start(int argc, char *argv[])
{
    ...
    TELEMETRY_INIT(IARM_BUS_DEEPSLEEPMGR_NAME);
...
static gboolean heartbeatMsg(gpointer data)
{
    ...
    TELEMETRY_EVENT_STRING("SYST_INFO_DS_HB", "I-ARM Deep Sleep Mgr: HeartBeat ping.");
    return TRUE;
}
			       ...
			       LOG("Error: Failed to execute alert system command (exit code: %d)\n", result);
                        ...
                        while(retryCount < MAX_RETRIES)
                        {
                            ...
                            if(status != 0)
                            {
                                ...
                                if(retryCount >= MAX_RETRIES)
                                {
                                    ...
                                    TELEMETRY_EVENT_STRING("SYST_ERR_DSModeFail", "ERROR: Device failed to enter into Deep sleep Mode after 5 retries, generate key event to transition to light sleep.");
                                    pthread_mutex_lock(&deep_sleep_mutex);
                                    IsDeviceInDeepSleep = DeepSleepStatus_Failed;
                                    pthread_mutex_unlock(&deep_sleep_mutex);
                        ...
                        /* Always send KED_DEEPSLEEP_WAKEUP when user action wakes the device from deep sleep. Previously this was sent
                           ...
                           is always sent here */
                            ...
                            TELEMETRY_EVENT_STRING("SYST_INFO_DS_WakeUp", "Resumed due to user action. Sending KED_DEEPSLEEP_WAKEUP.");
                            IARM_Bus_IRMgr_EventData_t eventData;
                            eventData.data.irkey.keyType = KET_KEYDOWN;
                            eventData.data.irkey.keyCode = KED_DEEPSLEEP_WAKEUP;
                            eventData.data.irkey.isFP = 0;
